<?php 
	...
	if ($_POST['_action_'] == FALSE) {
		...
		<form action="" id="registration_form" name="registration_form" method="POST">
			...
			<label for="username">Username:* <small>(Username mora imati min 5 i max 10 znakova)</small></label>
			<input type="text" id="username" name="username" pattern=".{5,10}" placeholder="Username.." required><br>
			
									
			<label for="password">Password:* <small>(Password mora imati min 4 znakova)</small></label>
		...
		if ($row['email'] == '' || $row['username'] == '') {
			...
			echo '<p>User sa ovim email-om ili usernam-om već postoji!</p>';
		}
	}
	print '
	</div>';
